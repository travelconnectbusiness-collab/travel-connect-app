...
async function submitLogin(inviteToken){
 ...
 const businessType=(role==="owner")?(document.querySelector("#loginBizType")?.value||"taxi_travel"):"";
 ...
 try{
  ...
  if(businessType) localStorage.setItem("tc_chosen_business_type",businessType); /* remembered so registration doesn't ask again */
 ...
 <div style="display:flex;align-items:center;justify-content:center;padding:30px 16px">
  <div style="background:#fff;border-radius:18px;max-width:360px;width:100%;padding:30px 26px;text-align:center;box-shadow:0 8px 24px rgba(0,0,0,.12)">
   ...
   <div id="loginBizTypeWrap" style="text-align:left;margin-bottom:14px">
    ...
    <select id="loginBizType" style="width:100%;padding:11px;border-radius:9px;border:1px solid #c9d4dc;font-size:15px;box-sizing:border-box">${tcBusinessTypeOptions()}</select>
   </div>
...
function renderPartnerRegisterForm(){
 ...
 <div class="grid">
  <label>Business type<select id="pBizType">${tcBusinessTypeOptions(localStorage.getItem("tc_chosen_business_type"))}</select></label>
