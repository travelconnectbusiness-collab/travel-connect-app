...
/* Properly asynchronous now, after three synchronous attempts here all
   failed - the QRCode library apparently finishes its actual canvas/image
   drawing some time after its constructor call returns rather than
   synchronously, and no synchronous read-back strategy tried so far
   reliably caught the finished result. Waiting two animation frames (the
   browser's own signal that two full render passes have completed) before
   reading the image back is a standard, reliable way to give a library's
